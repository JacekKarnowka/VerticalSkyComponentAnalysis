...
def generate_excel(n_clicks, data, option):
    global path_all
    if 'Excel_file' not in os.listdir('{}'.format(UPLOAD_DIRECTORY)):
        os.mkdir('{}/Excel_file'.format(UPLOAD_DIRECTORY))
    ...
    writer = pd.ExcelWriter('{}/Excel_file/Generated_Data.xlsx'.format(UPLOAD_DIRECTORY), engine='xlsxwriter')
    ...
    download1('../../../../Downloads/Generated_Data.xlsx')
    return 0
...
def open_files():
    ...
    path_all = r'{}/Data/Data/{}/{}-3d-VSC-APSH-template_Waldram/Reports'.format(urlquote(UPLOAD_DIRECTORY),
                                                                                 main_project_name[0], project_number)
